...
nodes = {
    ...
    "Z": Node("Z", (0, 5), 8, ["G", "B", "S","t"]),
    "B": Node("B", (1, 2), 7, ["Z", "D", "S"]),
    "A": Node("A", (2, 0), 5, ["M", "S"]),
    "M": Node("M", (3, 3), 4, ["G", "A","t"]),
    "D": Node("D", (4, 2), 6, ["G", "B"]),
...
async def find_path(request: PathRequest):
    # Reset all nodes to their original heuristics first
    for name, node in nodes.items():
        node.heuristic = original_heuristics[name]
    
    # Then set the goal node's heuristic to 0
...
async def delete_node(node_name: str):
    if node_name in nodes:
        # Remove the node from other nodes' connections
        for node in nodes.values():
